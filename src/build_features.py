"""End-to-end regeneration of data
...
3. Aggregate hourly (drugs summed, others averaged) to pre-LOCF table.
4. Label with CAM-ICU + RASS, applying prevalent-delirium,
   coma, missing-data, and negative-censoring gates (src.labeling).
5. Apply honest LOCF (src.features.densify_and_locf).

The IO/scan layer in main needs MIMIC-IV on Brown University's OSCAR.
...
def assemble_dataset(
    ...
    *,
    ...
    cam_events : full-stay stay_id, hour_offset, is_positive.
    rass_events : full-stay stay_id, hour_offset, rass_val.
    ...
    # Coma in first window (never arousable)
    ...
    # Hourly aggregation sum drugs average others
    hourly_prelocf = aggregate_hourly(fe, drug_feature_names=set(DRUG_FEATURES))

    # missing data in first window 
    ...
    # negative-class censoring using post window assessment
