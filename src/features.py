...
This module centralizes the feature-engineering logic that previously lived
inline in ``01_cohort_extraction.ipynb`` so it is importable, testable, and
consistent with :mod:`src.data.feature_vocab`.

Key fixes over the original notebook implementation:

* **Drug itemids resolved by label** against ``d_items`` at runtime
  (:func:`resolve_drug_itemids`) instead of hardcoded numbers. This repairs the
  two known-broken mappings — ``drug_ketamine`` (itemid not found) and
  ``drug_methadone`` (previously pointed at "Sheath Insertion") — and drops any
  drug that cannot be verified, with a report.
* **Honest LOCF** (:func:`densify_and_locf`): the hourly series is reindexed to a
...
* **Correct drug aggregation**: drug amounts are **summed** within an hour
  (cumulative dose), while chart/lab values are **averaged**.
* **Data-coverage gate** (:func:`stays_with_insufficient_data`) implementing the
  DeLLiriuM "exclude missing data in the first 24 h" criterion.

The MIMIC-scanning loaders are intentionally thin; the heavy lifting is in pure
functions operating on in-memory DataFrames so they unit-test without MIMIC.
"""
...
# ── Chart itemids (verified OK in the original d_items audit) ──────────────────
...
# ── Drug label patterns for robust, verifiable resolution against d_items ──────
# (include_regex, exclude_regex|None) matched case-insensitively on d_items.label.
# Lookbehind/exclusion patterns disambiguate (nor)epinephrine and fentanyl conc.
...
def resolve_drug_itemids(
    ...
    """Resolve drug feature names → itemids by matching ``d_items.label``.

    Parameters
    ----------
    d_items : DataFrame with at least ``itemid`` and ``label`` (and optionally
        ``linksto``; if present, restricted to ``inputevents``).

    Returns
    -------
    (itemid_to_name, report)
        ``itemid_to_name`` maps every resolved itemid to its drug feature name
        (a feature may map from multiple itemids). ``report`` has one row per
        drug feature: ``feature_name``, ``n_itemids``, ``itemids``, ``labels``,
        ``status`` ("ok" / "NOT FOUND").
    """
...
def densify_and_locf(hourly: pd.DataFrame, *, max_hours: int = 24) -> pd.DataFrame:
    ...
    This is the "LOCF" the documentation promises: gaps *between* observed hours
    are filled by carrying the last value forward. Hours before the first
    observation stay missing and are dropped (no backfill). Returns long format
    (``stay_id``, ``hour_offset``, ``feature_name``, ``value``).
    """
    if hourly.empty:
        return hourly.copy()
    parts = []
...
# ── data-coverage gate (DeLLiriuM "no missing data in first 24h") ──────────────
