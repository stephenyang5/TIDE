...
This module is the **single source of truth** for the delirium label. It
replaces the two divergent definitions that previously existed (the ICD-based
label in ``build_cohort.py`` and the ad-hoc ±2 h CAM/RASS matching in
``01_cohort_extraction.ipynb``).
...
Cohort gates derived from the same chart signals:

  * **Prevalent delirium** — any CAM+ within the first 24 h ⇒ exclude (we want
    to predict *onset*, not screen prevalent cases).
...
All functions are *pure* (operate on in-memory DataFrames) so they can be unit
tested on synthetic data without MIMIC access. ``load_*`` helpers that scan the
raw MIMIC ``chartevents`` table live in ``src/cohort_features`` callers /
notebook, keeping this module import-light and testable.
"""
...
def assessed_after_window(
    ...
    *,
    ...
    12 h post-window intervals containing a CAM-ICU assessment (positive *or*
    negative; RASS readings also count if ``rass_events`` is given).

    This underpins the **negative-class censoring** fix: a stay may only be a
    trustworthy ``label == 0`` if it was assessable and assessed after 24 h —
    otherwise its "negative" is really just absent follow-up.
    """
    frames = []
...
def label_delirium(
    ...
    *,
    ...
    """Assign the DeLLiriuM delirium label to each stay.
    ...
    A stay is positive iff there exists a 12 h assessment interval (starting
    after ``prediction_window_hours``) containing **both** a CAM+ event and a
    RASS ≥ ``rass_assessable_min`` reading.
    """
